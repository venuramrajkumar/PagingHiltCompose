    ...
    override suspend fun load(
        ...
        return try {
            val currentPage = when (loadType) {
                LoadType.REFRESH -> {
                    //REFRESH type is when we request data for the first time
                    val remoteKeys = getRemoteKeyClosestToCurrentPosition(state)
                    remoteKeys?.nextPage?.minus(1) ?: 1
                    // TO get current page number we are removing 1 from nextPage number.
                // when remoteKey/nextPage is null it returns 1
                }
                LoadType.PREPEND -> {
            ...
            //when ever we want to perform multiple db operations in parellel use withTransaction
